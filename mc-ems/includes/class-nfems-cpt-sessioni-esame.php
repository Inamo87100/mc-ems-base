...
if (!$course_pt) {
    ...
    echo '<select name="nfems_course_id" required><option value="0">— Select course —</option>';
...
echo '</td></tr>';
        ...
        printf('<input type="date" id="nfems_date_input" name="nfems_date" value="%s" min="%s" />', esc_attr($date), esc_attr(current_time('Y-m-d')));
        echo '</td></tr>';

        echo '<tr><th><label>Time</label></th><td>';
        printf('<input type="time" id="nfems_time_input" name="nfems_time" value="%s" />', esc_attr($time));
        echo '</td></tr>';
        ...
        printf('<input type="number" min="1" max="500" name="nfems_capacity" value="%d" %s />',
            (int)$capacity,
            ($is_spec ? 'readonly' : '')
        );
        ...
        echo '<select name="nfems_proctor_user_id"><option value="0">— None —</option>';
        foreach ($users as $u) {
            printf(
        ...
        printf('<label><input type="checkbox" name="nfems_is_special" value="1" %s /> ♿ Session for special requirements</label>',
            checked($is_spec, 1, false)
        );
        echo '</td></tr>';

        echo '<tr><th><label>Associated candidate (only ♿)</label></th><td>';
        echo '<select name="nfems_special_user_id"><option value="0">— None —</option>';
        foreach ($users as $u) {
            printf(
                '<option value="%d" %s>%s (%s)</option>',
                (int)$u->ID,
